...
dataset, vocab_size = pipeline.prepare_dataset(CSV_PATH)

train_loader = DataLoader(dataset, batch_size=BATCH_SIZE, shuffle=True)

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
model = ReasoningModel(vocab_size, EMBED_DIM, HIDDEN_DIM, DROPOUT).to(device)

optimizer = torch.optim.Adadelta(model.parameters(), lr=1.0)
criterion = nn.CrossEntropyLoss() 

print(f"\nModel Created. Device: {device}")
print(f"Input Batch Shape: [B, 4, Seq]")

print("\n--- Testing One Training Step ---")

ctx_batch, hyp_batch, label_batch = next(iter(train_loader))
ctx_batch, hyp_batch, label_batch = ctx_batch.to(device), hyp_batch.to(device), label_batch.to(device)
...
BATCH_SIZE = 16          # Paper used 10 
LEARNING_RATE = 1.0      # Paper used 1.0 with Adadelta 
L2_REG = 1e-5            # Paper used 10^-5 
DROPOUT = 0.3            # Paper used 0.3 for embeddings [cite: 151]
...
pipeline = TextPipeline() # Using the class we defined in the previous step
full_dataset, vocab_size = pipeline.prepare_dataset(CSV_PATH)

# Split Training and Validation (Simulating the 800 val set from paper)
val_size = 800 
...
# Loss Function: Standard for Multiple Choice (0, 1, 2, 3)
criterion = nn.CrossEntropyLoss()

# Optimizer: Adadelta as per paper
optimizer = torch.optim.Adadelta(
    model.parameters(), 
    lr=LEARNING_RATE, 
    weight_decay=L2_REG # This implements the L2 Regularization
)

# --- 4. HELPER FUNCTION: ACCURACY ---
def calculate_accuracy(preds, labels):
    # preds shape: [Batch, 4] -> Get index of highest score
...
for epoch in range(NUM_EPOCHS):
    ...
    for ctx_batch, hyp_batch, label_batch in progress_bar:
        ...
        logits = model(ctx_batch, hyp_batch) # Shape: [Batch, 4]
        ...
        # Optional: Clip Gradients to prevent exploding gradients in LSTM
