...
class RandomEmbeddingEncoder(nn.Module):
    def __init__(self, config):
        ...
        # 1. Embedding Layer
        self.embedding = nn.Embedding(config.vocab_size, self.embedding_dim)
        
        # 2. Projection to match DAGN hidden size
        self.projection = nn.Linear(self.embedding_dim, self.hidden_size)
        
        # 3. Layer Normalization (CRITICAL FIX)
        # Transformers like RoBERTa apply LayerNorm to their outputs. 
        # Since we removed RoBERTa, we must add this manually to stabilize 
        # the inputs before they reach the Graph Network.
        self.LayerNorm = nn.LayerNorm(self.hidden_size, eps=1e-12)
        
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        
        # 4. Explicit Initialization (CRITICAL FIX)
        self.init_weights()

    def init_weights(self):
        # Initialize embeddings with small std dev (0.02) to match BERT/RoBERTa
    ...
    def forward(self, input_ids, attention_mask=None, token_type_ids=None):
        ...
        sequence_output = self.LayerNorm(projected_embeddings)
        ...
        if attention_mask is not None:
             ...
             # Use a safer large negative number than -1e9 to avoid potential NaNs in older pytorch versions
             # though -1e9 is generally safe.
             masked_sequence_output = sequence_output * mask_expanded + (1 - mask_expanded) * -1e9
...
class DAGN(BertPreTrainedModel):
    ...
    def __init__(self,
        ...
        if token_encoder_type == "lstm":
            self.roberta = BiLSTMEncoder(config)
        elif token_encoder_type == "random":
            self.roberta = RandomEmbeddingEncoder(config)
        else:
